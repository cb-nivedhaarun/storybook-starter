<template>
 <div>
    <div class="border-b-2 py-2 mb-5">
      <h1 class="text-2xl	text-gray-700">Chargebee Social Icons - {{ variant }}</h1>
    </div>
    <div class="grid grid-cols-2">
        <div class="demo-wrapper border-r-2 max-h-screen overflow-scroll">
            ...
            <div v-if="variant === 'inverted'" id="inverted" class="demo grid grid-cols-3 gap-5">
                <template v-for="(item, index) in cssArr">
                  <div :key="index" class="p-3 bg-black background cursor-pointer" @click="setIndex">
                    <div :style="item.style" :id="index"></div>
                  </div>
                </template>
            </div>
    ...
    <!-- <div class="cb-storybook-css__razorpay--primary"></div>
    <div class="cb-storybook-img__gocardless--primary"></div> -->
 </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    let data = imageData.filter((item) => (item.category == "chargebee-logo" && item.variant === self.variant))
